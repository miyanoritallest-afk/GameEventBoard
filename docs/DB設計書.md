...
- event_series / series_members: series_members(active) のみ更新可。owner のみ運営追加削除・シリーズ削除。参照は公開。
  - 実装状況: 0032 で設定（シリーズ PR-⑥-1）。event_series は SELECT=公開／INSERT=本人（created_by=auth.uid()）／UPDATE=owner のみ。series_members は SELECT=公開／INSERT=作成者本人が自分を owner・active で登録するときのみ（招待は⑥-2）。owner/staff 判定は多段のため security definer 関数 `is_series_owner` / `is_series_staff` に切り出し（0015 と同型）。シリーズ作成＋owner登録は **`create_series_with_owner` security definer 関数で1トランザクション**（分割 INSERT だと owner 不在の孤立シリーズが残るため）。events.series_id への紐付けはシリーズ化 Action（seriesifyEvent）と新回作成のプリフィルから（events_update_own / 0004 が最終防衛）。③公開通知は主催者フォロワー ∪ series フォロワーを宛先集約する。
    - **0033（シリーズ PR-⑥-2・共同運営）**: series_members に招待/承認/削除を追加。多段判定・原子性・不変条件は security definer 関数に寄せる: `search_users_for_invite`（招待候補検索・既member除外・上限20。users の他人行は RLS 非公開のため definer で跨ぐ）／`invite_series_member`（owner資格・二重招待防止を検証し admin・invited で INSERT）／`respond_to_series_invite`（本人の invited 行のみ承認/拒否）／`remove_series_member`（owner資格＋**最後の active owner 保護**＝シリーズ孤立防止）。#11 招待通知（`series_member_invited`）は直接関係者（招待相手1人宛・フォロー集約不要）。
    - **0034（シリーズ PR-⑥-2 修正・認可バイパス対策）**: 0033 の definer 関数は actor を引数（p_inviter 等）で受け取っており、Postgres の PUBLIC EXECUTE 既定付与のため REST 直叩きで**他人の owner UUID を渡して RLS をバイパスし権限昇格できた**（実地再現）。修正: (1) 4関数の actor を **`auth.uid()` に変更・引数廃止**。(2) `search_users_for_invite` に呼び出し元 owner チェック追加＋ilike の LIKE メタ文字（`% _`）を `escape` 句でエスケープ。(3) 招待/承認/削除は**全て definer 関数経由に強制**し、series_members の INSERT/UPDATE/DELETE の RLS ポリシー（0033 で足した3本）を**撤去**（RLS 有効＋ポリシー無し＝直 REST 書き込みは全拒否）。最後 owner 保護は関数内、TOCTOU は `for update` で担保。(4) 4関数の EXECUTE を **anon から REVOKE**（authenticated のみ）。→ **RLS/認可の主役が「関数のみが書ける」形に一本化**され、閲覧は SELECT 公開のまま。
...
- follows / notifications: 本人のみ。
  ...
  - 1日1回集約（event フォロワー・#5/#6）: 0031 で `notification_events.dedup_key`（nullable・unique）＋ `upsert_notification_event(...)` security definer 関数を追加。結果更新・日程更新は同じイベントで何度も起きるため、dedup_key=`event:<id>:<type>:<JST日付>` で「1イベント・1種別・1日1出来事」を DB 物理保証（同日2回目は既存行を再利用→同じ source_event_id→notifications の UNIQUE が2通目を弾く）。dedup_key=null の一回性出来事（#1/#4）は従来どおり毎回新規行。notification_events は SELECT 不可のため find-or-create は security definer 関数で行う。
  ...
  - notification_events / notification_deliveries 実装状況: 0027 で設定。ともに **SELECT ポリシーを意図的に作らない**＝一般ユーザーは出来事・配信状況テーブルを直接読めない（UI に出さないサーバー処理専用データ）。INSERT のみ authenticated（Server Action / 将来の宛先集約・Discord 配信から記録）。
    - **④ Discord Webhook（全体告知）実装状況（通知 PR-④）**: マイグレーション不要（既存スキーマで完結）。`events.discord_webhook_url`（イベント作成/編集フォームで入力・Zod で **Discord ホストの webhook 形式のみ受理**）に投稿先を持ち、公開時に告知チャンネルへ Webhook POST（`postToDiscordWebhook`・app サービス層・ベストエフォート）。結果を `notification_deliveries`（channel=`discord_webhook`・status=`sent`/`failed`/`skipped`・**target_ref はホストのみ＝トークン非保存**）へ INSERT（0027 のポリシーで足りる）。全体告知は宛先集約せず**チャンネルに1投稿**（③のフォロワー個別通知とは別レイヤー）。Discord メッセージ内リンクは絶対URL（`NEXT_PUBLIC_APP_URL`）。
    - **⑦ Cron 定期通知 実装状況（通知 PR-⑦）**: マイグレーション不要（`matches.scheduled_at`/`notified_at` は 0001 既存）。Vercel Cron が `/api/cron/notifications`（Route Handler・**CRON_SECRET の Bearer 検証**で保護）を叩く。Cron は**ログインセッションが無い**ため RLS を越えられず、**service_role クライアント（`createAdminClient`・RLS 全バイパス）**で他人の試合・メンバーを跨いで集約する（保護は CRON_SECRET が担う）。#7 試合直前リマインド＝2時間以内開始&`notified_at is null` の試合を拾い、出場メンバー（matches→teams→team_members→registrations→users）へアプリ内通知＋`notified_at` を**条件付き UPDATE** で立て二重送信を物理防止。#9「本日◯時から各試合」＝本日 JST に試合がある webhook 付き公開イベントを④の Webhook で告知し、`notification_events.dedup_key`（日付入り）で**1日1回**に集約。
  ...
  - **観戦ビュー Realtime（0036）**: `matches` / `match_results` を `supabase_realtime` publication に追加。RLS 追加は不要（両テーブルの anon SELECT は 0023 で公開イベント向けに開放済み）＝購読者は**匿名観戦者でも**公開イベントの変更だけを受け取る（下書きは `is_public_event` で弾かれ配信されない）。観戦ページのクライアントは `matches`（`event_id` で絞る）と `match_results`（event_id 列が無いため無フィルタ購読＋クライアントで当該イベントの `match_id` 集合に間引く）を購読し、変更検知で `router.refresh()`。購読者トークンは subscribe 前に `setAuth`（匿名時は anon キーの JWT）。
    - **⚠️ security definer 関数の認可修正（0037）**: 0034 の series 認可修正から取り残された2関数を修正。`create_series_with_owner`（0032）は actor を `p_created_by` 引数で受け＋EXECUTE=PUBLIC のため、REST 直叩きで**他人名義（owner）のシリーズを作れた**（0034 と同型の権限昇格）。→ 引数を廃止し関数内 `auth.uid()` を使う・anon から EXECUTE REVOKE（authenticated のみ）。`upsert_notification_event`（0031）は EXECUTE=PUBLIC のため任意ユーザーが `notification_events` に任意行を作れた（dedup_key 占有で正規通知を抑止/汚染）。→ サーバー処理専用なので anon/authenticated 双方から EXECUTE REVOKE。**教訓: definer 関数を1本直したら同時期に作った他の書き込み definer も EXECUTE 管理が揃っているか横断確認する**。

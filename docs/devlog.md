...
## 2026-08-18 — OW ランクにエメラルド追加（40段階→45段階）・既存データのずれ方を確定

OVERWATCH に新ランク **エメラルド** が新設された（序列はプラチナとダイヤの間）ため、ランク体系を **8帯40段階 → 9帯45段階** に拡張した。あわせて「既存イベントのデータが壊れないか」を調査し、影響範囲と対処方針を確定した。

### やったこと

- **影響調査**。ランク定義は `src/lib/services/overwatch-ranks.ts`（TS）と `supabase/seed.sql`（DB）に**同一規則が二重定義**され、アプリは TS 側を参照していることを確認。DB の `rank_definitions` は cron のキープアライブ SELECT 以外から読まれていない。`user_season_ranks` も未使用。
- **PR-1（コード）**: `OVERWATCH_TIERS` にエメラルドを `ord=4` で挿入、`TIER_ABBREV` に `E` を追加。スコア式 `ord*5 + (5-division) + 1` は**変更なし**（帯を1つ挿すだけで45段階になる）。`teamScoreCap` の上限を 40→45（`schema.ts` / `event-form.tsx`）。
- **マイグレーション 0038**: 既存 DB の再採番。`seed.sql` は `on conflict (game_id, label) do nothing` のため**既存40行の score を更新しない**ので、「ダイヤ以上を一律 +5 → エメラルド 21〜25 を挿入」を UPDATE で行う。エメラルド未挿入時のみ実行して二重適用を防ぐ。
- **テスト**: 45段階の固定に加え、エメラルドの位置（プラチナ1=20 → E5=21 … E1=25 → ダイヤ5=26）・帯序列・0038 の +5 再採番との一致を検証。391 tests 全緑。
- **ドキュメント更新**: DB設計書 3.5（45段階・0038 の再採番・スナップショット非追従の注意）/ ER図（`1..40`→`1..45`）/ 要件定義書 3.1.1・3.3・3.4。

### 決めたこと（なぜ）

- **スコアレンジは素直に 1〜45 にする**。「プラチナとダイヤの間に小数を差し込んで既存値を守る」案は、既存データはズレないが体系が汚れ、次にランクが増えたとき破綻するため不採用。
- **既存の応募スコアは書き換えない**（意図的な非追従）。`registrations.individual_score` / `final_score` / `events.team_score_cap` は**マスタ参照ではなく数値スナップショット**で保存されている。これは「後でランクが変動しても当時の振り分け記録が変わらないようにする」という元々の設計方針（要件定義 3.1.1）によるもの。結果として旧データは同じ数値が一段低い帯として表示されるが、これは仕様として受け入れる。
- **再計算での救済はしない**。デモイベント70件の `score_breakdown` は全件 `{"demo":true}` で `grid`（算出元のランクセル）を持たないため、そもそも再計算の材料がない。同時に、応募一覧の「算出根拠」表示は元から出ていないので、今回のずれによる誤表示も発生しない。
- **要件定義書の「コード直書きせず DB マスタで保持」を実態に合わせて訂正**。当初の意図と実装が乖離しており、今回まさに「データ追加のみでは済まずコード改修を要した」実例になった。設計書と実装の食い違いを放置せず、経緯ごと残す形で補足を書いた（3.4「コード改修なしでデータ追加のみ」も未達である旨を明記）。
- **`peak_tier` enum は変更なし**。到達ボーナスはマスター以上が対象でエメラルドは無関係。

### 次にやること

- **PR-3**: デモイベント「Matchpoint Open Vol.1」のスコア再分配。現状の分布は 15〜30（＝半数以上が旧ダイヤ帯以上）で、45段階ではエメラルド帯として表示される。ポートフォリオとして**エメラルド帯の参加者が実在する**見え方にするため、70名のスコアを振り直し、`team_score_cap`（現 26）も併せて調整する。順位・チーム編成は確定済みなので、上限との整合を崩さない値にすること。

## 2026-07-18 — 理解力チェック用「問題集」を作成（学習の棚卸し）

講座の最終課題としてアプリ開発を通じて身につけたスキルを整理・確認するため、**理解力チェックの問題集**（[docs/問題集.md](./問題集.md)）を作成した。別プロジェクト RaiseChat の問題集（技術教科書 第10章）と同じ体裁に合わせた。

### やったこと

- **`docs/問題集.md` を新規作成**（全23問・4段階）。RaiseChat の「基礎 → 応用 → 設計 → チャレンジ」構成・`<details>` ヒント・コードスニペット問題の体裁を踏襲。
- 各問を GameEventBoard の**実コード・実設計判断に接地**させた。主な題材：3層構造 / Server・Client の使い分け / RLS / JST→UTC→JST / Zod マスアサインメント対策 / 操作系の防御段取り・IDOR（2テーブル跨ぎ・列挙防止）/ 楽観ロック＋条件付きUPDATE / Realtime `setAuth` / 通知の重複排除・`allSettled` / スコア未認定補完3方式 / security definer ハードニング（0037）/ 技術選定（Prisma不採用・2層認可・ランク非永続・チームスコア非保存・非常駐Bot）/ 拡張課題（ランク収集2軸・同ロール交代・Cronリマインド・スケール）。

### 決めたこと（なぜ）

- **模範解答は載せない**（ユーザー選択）。「自分の言葉で答えを作る」ことが学習の本質のため、答えは各自 devlog / 設計ドキュメントを見て組み立てる方針。各問には代わりに `<details>` ヒントと参照先ドキュメントのリンクを付けた。
- **難易度は RaiseChat と同じ4段階バランス**。設計問題・チャレンジ問題を厚めにし、面接での「自分の言葉での説明」を鍛える。
- ドキュメントは実態から作る規律を守り、既存の設計ドキュメント（アーキテクチャ設計書・スコアリング設計・実装ガイドライン）と実コード（`scoring.ts` / `team-score.ts` / `actions.ts` / `teams.ts` / migrations 0004・0037）を照合してから出題した。

### 次にやること

- 実際に問題を解いてみて、答えに詰まる箇所（説明が弱いスキル）を洗い出す。必要なら該当ドキュメントを補強する。

## 2026-07-16 — 本番デプロイ（Vercel + 本番 Supabase）・公開に伴う調整4件

アプリを **Vercel + 本番 Supabase で本番公開**した。CD（`main` へのマージで自動デプロイ）も有効化。公開作業に付随して、Cron のプラン制限対応・ゲーム名の誤記統一・README のデモ素材差し替えを行った。ポートフォリオとして提出できる状態に到達。

### やったこと（PR単位・時系列）

1. **本番デプロイ（#121）**: 開発とは別の本番専用 Supabase を構築（マイグレーション37本を番号順適用・マスタ seed・API キー取得）→ Vercel に GitHub リポジトリを接続してデプロイ。**接続した瞬間から CD が有効**になり、以降 `main` への push/merge で自動ビルド&デプロイされる。初回向けに詳しい **[デプロイ手順書](./デプロイ手順.md)** を用意（①本番 Supabase → ②Vercel 接続 → ③環境変数・OAuth 突き合わせ → ④デモデータ）。採用担当が実際に触れるよう、本番用のデモ大会 seed（`supabase/prod_seed_demo_event.sql`）も追加。
2. **Vercel Cron を日次に変更（#122）**: デプロイ後、Hobby（無料）プランでは Cron が**日次までしか許可されない**制限に当たった。通知生成 Cron（`/api/cron/notifications`）の schedule を `0 0 * * *`（毎日 0:00）に変更。より短い間隔は有料プランが必要なため、無料枠に合わせた。
3. **ゲーム名の誤記統一（#123）**: ランディングページの表示が「OVERWATCH 2」になっていた（正しくは「OVERWATCH」＝2023年のリブランディングで正式名称が戻っている）。**ユーザー可視の `page.tsx`** を修正し、併せてコードコメントの「OW2」略記も5ファイルで「OW」に統一。
4. **README のデモ素材更新（#124）**: チーム編成 D&D の GIF を撮り直し版に差し替え（本番URLで撮影・チーム名/平均スコアが鮮明・3.6MB→2.7MB）。観戦ビューの説明文を実際のGIF（大会全体をスクロールで通覧）に合わせて修正し、見出しを「大会の全体像を1ページで」に。Realtime ライブ更新は実装済みのため機能として併記。

### 決めたこと（なぜ）

- **本番 Supabase は開発用と完全に別プロジェクト**にする。本番データと開発の実験を混ぜない。マイグレーションは番号順に手動適用（[[migration-apply-practice]]）。
- **Cron は無料枠（日次）に合わせる**。通知は日次生成でも用途上十分で、この段階で有料プランに上げる必要はない。運用しながら頻度が要れば見直す。
- **README のデモ文言は「GIFが実際に写しているもの」に一致させる**。Realtime のライブ更新は撮影手段がなかったため、GIFはスクロールでの全体通覧にした。文言を実態に寄せつつ、Realtime は実装済み機能として正直に併記（誇張しない）。

### 確認

- **本番デプロイ成功**（Vercel が本番URLを発行・アプリが公開状態で動作）。マージ→自動デプロイの CD フローも実際に機能することを #123/#124 のマージで確認。
- 誤記統一・GIF差し替えともに `npm run check`（lint 0・typecheck・387 tests）通過。src 内に「OVERWATCH 2」「OW2」の残存なし。

### 次にやること

- 本番公開は完了。区切りとして一段落。以降は運用で気づいた点の調整や、追加機能があれば feature ブランチで対応する。
